...
ADRCController::ADRCController(
    ...
    __u(r.size(), 0.0)
{   
    if (nlsef_alpha1.empty()) {
        __alpha1 = vector<double>(r.size(), 0.9);
    }
    else {
        __alpha1 = nlsef_alpha1;
    }
    if (nlsef_alpha2.empty()) {
        __alpha2 = vector<double>(r.size(), 1.5);
    }
    else {
        __alpha2 = nlsef_alpha2;
    }
    if (control_limits.empty()) {
        __control_limits = vector<pair<double, double>>(r.size(), { -Infinity, Infinity });
    }
    else {
        __control_limits = control_limits;
    }
    if (r.size() != b0.size() || r.size() != delta.size() || r.size() != eso_beta01.size() || 
        r.size() != eso_beta02.size() || r.size() != eso_beta03.size() || r.size() != nlsef_alpha1.size() || 
        r.size() != nlsef_alpha2.size() || r.size() != nlsef_beta1.size() || r.size() != nlsef_beta2.size()) {
        throw std::invalid_argument("Dimensional inconsistency!");
    }
    for (double alpha1 : __alpha1) {
        if (alpha1 >= 1 || alpha1 <= 0) {
            throw std::invalid_argument("0 < alpha1 < 1");
        }
    }
    for (double alpha2 : __alpha2) {
        if (alpha2 <= 1) {
            throw std::invalid_argument("alpha2 > 1");
        }
    }
